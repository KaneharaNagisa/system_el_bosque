...
function getBaseAvailability(year: number, month: number, day: number): AvailabilityStatus {
...
function getAvailability(year: number, month: number, day: number): DisplayStatus {
  ...
  for (let i = 1; i <= lookBack; i++) {
    ...
    const prevBase = getBaseAvailability(prev.getFullYear(), prev.getMonth(), prev.getDate());
    if (prevBase === "booked") {
      return "blocked";
    }
  }
...
const STATUS_LABELS: { status: DisplayStatus; label: string; symbol: string }[] = [
  ...
  const calendarData = useMemo(() => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      cells.push({
        ...
        year: YEAR,
        isCurrentMonth: true,
        status: getAvailability(YEAR, viewMonth, d),
    ...
    >
          ...
          >
            <span
              style={{
                ...
                fontWeight: 700,
                fontSize: status === "unavailable" ? "0.65rem" : "0.8rem",
                width: "1.2em",
                textAlign: "center",
              }}
            >
      ...
      >
        {calendarData.map((cell, idx) => {
          ...
          const isUnavailDay = isCurrentMonth && status === "unavailable";
          ...
          const isCheckoutCandidate =
            isSelectingCheckout && validCheckoutDates.has(dateStr) && isCurrentMonth;
          ...
          if (isCI || isCO) {
            ...
            bg = "repeating-linear-gradient(-45deg, rgba(138,122,106,0.06), rgba(138,122,106,0.06) 3px, rgba(138,122,106,0.02) 3px, rgba(138,122,106,0.02) 6px)";
          } else if (isBlockedDay) {
            bg = "repeating-linear-gradient(-45deg, rgba(184,134,11,0.05), rgba(184,134,11,0.05) 3px, rgba(184,134,11,0.015) 3px, rgba(184,134,11,0.015) 6px)";
          } else if (isCurrentMonth && status !== "closed") {
            bg = STATUS_BG[status];
          }

          const isStripedBg = (isUnavailDay || isBlockedDay) && !isCI && !isCO && !(inRange && isCurrentMonth);

          // Border for checkout candidates
          const borderStyle = isCheckoutCandidate && isBookable
            ? "2px solid #2d6a1e"
          ...
          return (
            <button
              ...
              style={{
                ...
                opacity: isUnavailDay ? 0.6 : isBlockedDay ? 0.7 : 1,
              }}
              onMouseEnter={(e) => {
            ...
            >
              <span
                style={{
                  ...
                  color: dayColor,
                  lineHeight: 1.2,
                  textDecorationLine: isUnavailDay ? "line-through" : "none",
                  textDecorationColor: "rgba(138,122,106,0.5)",
                }}
              >
                {isCurrentMonth ? day : <span style={{ opacity: 0.35 }}>{day}</span>}
              </span>
              {isCurrentMonth && (
                <span
                  style={{
                    fontSize: status === "unavailable" ? "0.55rem" : "0.62rem",
                    fontWeight: 700,
                    color: isCI || isCO ? "#f0e8d0" : symbolColor,
                    lineHeight: 1,
                    marginTop: "1px",
                  }}
                >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
          ...
          <span style={{ fontSize: "0.72rem", color: "#5a4838" }}>
            空き <strong style={{ color: "#2d6a1e" }}>{summary.available}</strong> 日
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
          <FaCircle size={7} color={STATUS_COLORS.booked} />
          <span style={{ fontSize: "0.72rem", color: "#5a4838" }}>
            予約済 <strong style={{ color: "#a03020" }}>{summary.booked}</strong> 日
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
          <FaCircle size={7} color={STATUS_COLORS.blocked} />
          <span style={{ fontSize: "0.72rem", color: "#5a4838" }}>
            清掃 <strong style={{ color: "#b8860b" }}>{summary.blocked}</strong> 日
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
          <FaBan size={7} color={STATUS_COLORS.unavailable} />
          <span style={{ fontSize: "0.72rem", color: "#5a4838" }}>
            不可 <strong style={{ color: "#8a7a6a" }}>{summary.unavailable}</strong> 日
          </span>
        </div>
      </div>
