...
const SectionHeading = ({
  ...
  <div style={{ textAlign: "center", marginBottom: "3rem" }}>
    ...
    {note && (
      <p style={{ color: dark ? "rgba(240,232,208,0.6)" : "#7a6858", marginTop: "0.75rem", fontSize: "0.85rem" }}>
        {note}
      </p>
    )}
  </div>
...
const experienceOptions = [
  { label: "田植え・稲刈り体験（¥4,500/人）", perPerson: 4500, flat: 0 },
  { label: "薪割り体験（¥2,000/1時間）", perPerson: 0, flat: 2000 },
  { label: "夏野菜収穫体験（¥1,500/カゴ）", perPerson: 0, flat: 1500 },
  { label: "BBQグリルレンタル（¥3,500）", perPerson: 0, flat: 3500 },
];

export function Pricing() {
  const [dayType, setDayType] = useState<"weekday" | "weekend" | "special">("weekday");
  const [guests, setGuests] = useState(2);
  const [petFee, setPetFee] = useState(0);
  const [selectedExperiences, setSelectedExperiences] = useState<number[]>([]);
  const [supportEnabled, setSupportEnabled] = useState(false);

  const baseMap = { weekday: 20000, weekend: 26000, special: 33000 };

  const base = baseMap[dayType];
  const guestExtra = guests > 5 ? (guests - 5) * 3000 : 0;
  ...
  const total = base + guestExtra + supportFee + transferSurcharge + petFee + experienceFee + 10000;
  ...
  const priceExamples = [
    {
      ...
      rows: [
        { label: "基本宿泊料（1〜5名・平日）", value: 20000 },
    ...
    },
    {
      ...
      rows: [
        { label: "基本宿泊料（1〜5名・休前日）", value: 26000 },
    ...
    },
    {
      ...
      rows: [
        { label: "基本宿泊料（1〜5名・特別日）", value: 33000 },
        { label: "人数追加料金（6〜8名・3名分）", value: 9000 },
  ...
  const cancelPolicies = [
    ...
    { timing: "7日前〜4日前", rate: "宿泊料の 50%", rateNum: 50, free: false },
    { timing: "3日前〜2日前", rate: "宿泊料の 80%", rateNum: 80, free: false },
    { timing: "前日・当日・無連絡", rate: "宿泊料の 100%", rateNum: 100, free: false },
  ];
  ...
  return (
    <div>
        ...
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <div className="rate-table-wrap">
            ...
            <div className="rate-header-row">
              ...
              <span style={{ flex: 1, textAlign: "center" }}>1〜5名（基本）</span>
              <span style={{ flex: 1, textAlign: "center" }}>6名以上（1名追加につき）</span>
            </div>
            {[
              { label: "平日", sub: "日〜木", base: "¥20,000", highlight: false },
              { label: "休前日", sub: "金・土", base: "¥26,000", highlight: true },
              { label: "特別日", sub: "GW・お盆・年末等", base: "¥33,000", highlight: false },
            ].map((row) => (
              <div
                key={row.label}
                className="rate-data-row"
                style={{ backgroundColor: row.highlight ? "rgba(27,47,14,0.05)" : "transparent" }}
              >
                {/* 曜日区分 */}
                <div className="rate-cell-label">
                  <span className="rate-day-main">{row.label}</span>
                  <span className="rate-day-sub">（{row.sub}）</span>
                </div>
                {/* 基本料金 */}
                <div className="rate-cell-base">
                  <span className="rate-cell-mobile-label">1〜5名（基本）</span>
                  <span className="rate-base-value">{row.base}</span>
                </div>
                {/* 6名以上追加 */}
                <div className="rate-cell-extra">
                  <span className="rate-cell-mobile-label">6名以上（1名追加につき）</span>
                  <span className="rate-extra-value">+¥3,000 / 人</span>
                </div>
              </div>
            ))}
          </div>
          ...
          >
            ※ 1〜5名が基本料金。6名以上は1名追加につき¥3,000（曜日問わず一律）。最大10名まで宿泊可能です。
          </p>
      ...
      {/* ── ペット料金 ── */}
      <section style={{ backgroundColor: "#faf5e8", padding: "4rem 1.5rem" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
            ...
            >
              <thead>
                <tr style={{ backgroundColor: "#1b2f0e" }}>
                  ...
                  <th style={{ padding: "1rem 1.5rem", textAlign: "center", color: "#d4b070", fontSize: "0.82rem", fontWeight: 700 }}>
                    料金（1泊）
                  </th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr
                    key={row.label}
                    style={{ borderBottom: i < 3 ? "1px solid rgba(180,140,80,0.18)" : "none" }}
                  >
          ...
          <p style={{ fontSize: "0.8rem", color: "#8a7868", marginTop: "0.85rem", lineHeight: 1.7 }}>
              ...
              <tbody>
                {[
                  ...
                  <tr
                    key={row.name}
                    style={{
                      borderBottom: i < 4 ? "1px solid rgba(180,140,80,0.15)" : "none",
                      backgroundColor: i % 2 === 0 ? "transparent" : "rgba(27,47,14,0.03)",
                    }}
                  >
        ...
        <div style={{ maxWidth: "820px", margin: "0 auto" }}>
          ...
          >
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              {guests >= 5 && (
                <p style={{ fontSize: "0.72rem", color: "rgba(240,232,208,0.5)", marginTop: "0.4rem" }}>
                  ※ 5名以上のため送迎追加費¥5,000が含まれます（¥8,000 + ¥5,000）
                </p>
              )}
            </div>
            ...
            <div style={{ marginBottom: "2rem" }}>
              ...
              {selectedExperiences.some((idx) => experienceOptions[idx].perPerson > 0) && (
                <div
                  style={{
                    display: "flex",
                    alignItems: "center",
            ...
            >
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "1rem" }}>
                {[
                  { label: `基本宿泊料（${guests > 5 ? `1〜5名 + ${guests - 5}名追加` : "1〜5名"}）`, value: base + guestExtra },
                  ...(supportEnabled ? [{ label: "滞在サポート料", value: 8000 }] : []),
                  ...(transferSurcharge > 0
                    ? [{ label: "送迎追加（5名以上・一律）", value: transferSurcharge }]
                    : []),
                  ...(petFee > 0 ? [{ label: "ペット料金", value: petFee }] : []),
                  ...(experienceFee > 0 ? [{ label: "体験オプション", value: experienceFee }] : []),
                  { label: "保証料（返金制）", value: 10000 },
                ].map((row) => (
                  <div
                    key={row.label}
            ...
            {priceExamples.map((ex, i) => (
              ...
              >
                <div style={{ marginBottom: "1rem" }}>
                  ...
                  <p style={{ fontSize: "0.78rem", color: "#7a6858", lineHeight: 1.6 }}>{ex.desc}</p>
                </div>
                  ...
                  {ex.rows.map((row, j) => (
                    <div
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        padding: "0.4rem 0",
                        borderBottom: j < ex.rows.length - 1 ? "1px solid rgba(180,140,80,0.1)" : "none",
                        fontSize: "0.82rem",
                        color: "#5a4838",
                      }}
                    >
                      <span>{row.label}</span>
                      <span style={{ fontWeight: 600, color: "#2c1e10" }}>¥{row.value.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
                ...
                <div
                  style={{
                    ...
                    justifyContent: "space-between",
                    alignItems: "center",
                    borderTop: "2px solid rgba(196,122,48,0.35)",
                    paddingTop: "0.85rem",
                  }}
                >
                  <span style={{ fontSize: "0.82rem", color: "#5a4838", fontWeight: 600 }}>合計（税込）</span>
                  <span
                    style={{
                      fontFamily: "'Noto Serif JP', serif",
                      fontSize: "1.35rem",
      ...
      {/* ── ご注意事項 ── */}
      <section style={{ backgroundColor: "#f2e8d0", padding: "3rem 1.5rem" }}>
        <div style={{ maxWidth: "820px", margin: "0 auto" }}>
          <h3
            style={{
              fontFamily: "'Noto Serif JP', serif",
