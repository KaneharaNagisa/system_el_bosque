...
function formatPetDisplay(hasPet: string, petBreed: string, petBreed2?: string): string {
...
const mypageNews: NewsItem[] = [
  {
    ...
    content: "2回目以降のご利用の会員様には、滞在サポート料を特別割引にてご提供いたします。ご予約の備考欄に「リピーター割引希望」とご記入ください。詳しくはお問い合わせフォームよりご連絡ください。",
  ...
  },
  {
    ...
    content: "会員様限定で、2026年秋シーズン（9月〜11月）の先行予約を開始いたします。稲刈り体験・星空ガイドなど秋ならではのオプションもご用意しています。人気の連休はお早めにご予約ください。",
  ...
  },
  {
    ...
    content: "3月1日より2026年シーズンの営業を開始いたします。今シーズンも安心・快適な滞在をご提供できるよう、スタッフ一同心よりお待ちしております。皆様のご予約をお待ちしております。",
    publishDate: "2026-02-15",
  },
  {
    id: "NEWS-003",
    title: "GW期間の予約受付開始",
    content: "ゴールデンウィーク期間（4/29〜5/5）の予約受付を開始いたしました。特別料金期間となりますのでご注意ください。田植え体験もご予約いただけます。",
...
export function MyPage() {
  const { user, isLoggedIn, logout, updateProfile, deleteAccount } = useAuth();
  ...
  const [expandedNewsId, setExpandedNewsId] = useState<string | null>(mypageNews[0]?.id ?? null);
  ...
  const handleEditChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === "hasPet") {
      if (value === "no") {
        setEditForm((prev) => ({ ...prev, hasPet: value, petBreed: "", petBreed2: "" }));
      } else if (value !== "small2" && value !== "large2") {
        setEditForm((prev) => ({ ...prev, hasPet: value, petBreed2: "" }));
      } else {
  ...
  const parseAddress = (addr: string): { postalCode: string; address: string } => {
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                {mypageNews.map((item, idx) => {
                  ...
                  const isLast = idx === mypageNews.length - 1;
                  return (
                    <div
                      key={item.id}
                      style={{
                    ...
                    >
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "0.3rem", flex: 1, minWidth: 0 }}>
                        ...
                        <span style={{ color: "#8a7868", flexShrink: 0, marginTop: "0.2rem" }}>
            ...
            <>
                  ...
                  <button
                    onClick={() => {
                      setEditForm({
                        ...
                        lastNameKana: user.lastNameKana,
                        firstNameKana: user.firstNameKana,
                        postalCode: parseAddress(user.address).postalCode,
              ...
              {editing ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  ...
                  >
                    <p style={{ fontSize: "0.72rem", color: "#8a7868", marginBottom: "0.75rem", fontWeight: 600 }}>
                      ※ 以下の項目は変更できません
                    </p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }} className="mypage-edit-grid">
                      <div>
                        <label style={labelStyle}>メールアドレス</label>
                        <input value={user.email} disabled style={{ ...inputStyle, backgroundColor: "#eee5d5", color: "#8a7868" }} />
                      </div>
                      <div>
                        <label style={labelStyle}>生年月日</label>
                        <input
                          value={user.birthDate
                            ? user.birthDate.replace(/^(\d{4})-(\d{1,2})-(\d{1,2})$/, (_, y, m, d) => `${y}年${parseInt(m)}月${parseInt(d)}日`)
                            : "未登録"
                          }
                          disabled
                          style={{ ...inputStyle, backgroundColor: "#eee5d5", color: "#8a7868" }}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* ── Editable fields ── */}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }} className="mypage-edit-grid">
                    {/* お名前 */}
                    <div>
                      <label style={labelStyle}>姓</label>
                      <input name="lastName" value={editForm.lastName} onChange={handleEditChange} placeholder="例: 山田" style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>名</label>
                      <input name="firstName" value={editForm.firstName} onChange={handleEditChange} placeholder="例: 太郎" style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>姓（ふりがな）</label>
                      <input name="lastNameKana" value={editForm.lastNameKana} onChange={handleEditChange} placeholder="例: やまだ" style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>名（ふりがな）</label>
                      <input name="firstNameKana" value={editForm.firstNameKana} onChange={handleEditChange} placeholder="例: たろう" style={inputStyle} />
                    </div>
                    ...
                    <div>
                      ...
                      <select name="hasPet" value={editForm.hasPet} onChange={handleEditChange} style={selectStyle}>
                    ...
                    {(editForm.hasPet === "small" || editForm.hasPet === "large") && (
                      <div>
                    ...
                    {(editForm.hasPet === "small2" || editForm.hasPet === "large2") && (
                      <>
                        <div>
                    ...
                    <div style={{ gridColumn: "1 / -1" }}>
                      ...
                      <select name="hasFamily" value={editForm.hasFamily} onChange={handleEditChange} style={selectStyle}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                  {[
                    { label: "お名前", value: `${user.lastName} ${user.firstName}` },
                    { label: "ふりがな", value: `${user.lastNameKana} ${user.firstNameKana}` },
                    ...
                    { label: "生年月日", value: user.birthDate
                      ? user.birthDate.replace(/^(\d{4})-(\d{1,2})-(\d{1,2})$/, (_, y, m, d) => `${y}年${parseInt(m)}月${parseInt(d)}日`)
                      : "−" },
                    { label: "ペット同伴", value: formatPetDisplay(user.hasPet || "no", user.petBreed || "", user.petBreed2 || "") },
                    { label: "利用頻度の高い宿泊形態", value: formatFamilyDisplay(user.hasFamily || "") },
                    { label: "宿泊の際に気になること", value: user.concerns || "−" },
                    { label: "知ったきっかけ", value: user.howFound || "−" },
                    { label: "エルボスケに期待すること", value: user.expectations || "−" },
                  ].map((item) => (
                    <div
                      key={item.label}
                      style={{
                    ...
                    >
                      <span style={{ fontSize: "0.82rem", color: "#8a7868", minWidth: "100px" }}>{item.label}</span>
                      <span style={{ fontSize: "0.88rem", fontWeight: 500, color: "#2c1e10", textAlign: "right", flex: 1 }}>{item.value}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {sectionCard(
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                ...
                >
                  <div>
                    <h3 style={{ fontSize: "0.9rem", fontWeight: 700, color: "#2c1e10", marginBottom: "0.2rem" }}>
                      パスワード再設定
                    </h3>
                    <p style={{ fontSize: "0.75rem", color: "#8a7868", margin: 0 }}>
                      現在のパスワードを変更します
                    </p>
                  </div>
                  <Link
                ...
                >
                  <div>
                    <h3 style={{ fontSize: "0.9rem", fontWeight: 700, color: showDeleteConfirm ? "#a03020" : "#2c1e10", marginBottom: "0.2rem" }}>
                      会員退会
                    </h3>
                    <p style={{ fontSize: "0.75rem", color: "#8a7868", margin: 0 }}>
                      {showDeleteConfirm
              ...
              {mockReservations.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  {mockReservations.map((r) => {
                    const isCanceled = canceledIds.has(r.id);
                    const effectiveStatus = isCanceled ? "cancelled" : r.status;
                    const effectiveStatusLabel = isCanceled ? "キャンセル済" : r.statusLabel;
                    const sc = isCanceled
                      ? { bg: "rgba(160,48,32,0.06)", color: "#a03020", border: "rgba(160,48,32,0.2)" }
                      : statusColor(r.status);
                    const cancelable = !isCanceled && canCancel(r.checkin, r.status);
                    const isConfirmingCancel = cancelingId === r.id;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ padding: "1rem 1.25rem" }}>
                          ...
                          >
                            <div>
                              <span style={{ color: "#8a7868", fontSize: "0.7rem" }}>チェックイン</span>
                              <div style={{ fontWeight: 700, color: "#2c1e10", marginTop: "0.15rem" }}>
                                {formatDateJP(r.checkin)}
                              </div>
                            </div>
                            <div>
                              <span style={{ color: "#8a7868", fontSize: "0.7rem" }}>チェックアウト</span>
                              <div style={{ fontWeight: 700, color: "#2c1e10", marginTop: "0.15rem" }}>
                                {formatDateJP(r.checkout)}
                              </div>
                            </div>
                            <div>
                              <span style={{ color: "#8a7868", fontSize: "0.7rem" }}>人数</span>
                              <div style={{ fontWeight: 500, color: "#2c1e10", marginTop: "0.15rem" }}>
                                {r.guests}名
                              </div>
                            </div>
                            <div>
                            ...
                            <div>
                              <span style={{ color: "#8a7868", fontSize: "0.7rem", display: "flex", alignItems: "center", gap: "0.25rem" }}>
                          ...
                          {r.experiences.length > 0 && (
                            <div style={{ marginTop: "0.75rem", paddingTop: "0.6rem", borderTop: "1px solid rgba(180,140,80,0.12)" }}>
                              <span style={{ color: "#8a7868", fontSize: "0.7rem" }}>体験オプション</span>
                          ...
                          >
                            <span style={{ fontSize: "0.72rem", color: "#8a7868", fontWeight: 500, display: "flex", alignItems: "center", gap: "0.3rem" }}>
                          ...
                          {/* Cancel section */}
                          {cancelable && (
                            <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(180,140,80,0.12)" }}>
                              {isConfirmingCancel ? (
                                <div
                                  style={{
                                ...
                                >
                                  <p style={{ fontSize: "0.78rem", color: "#a03020", fontWeight: 700, marginBottom: "0.4rem" }}>
                                    この予約をキャンセルしてよろしいですか？
                                  </p>
                                  <p style={{ fontSize: "0.72rem", color: "#8a7868", marginBottom: "0.75rem", lineHeight: 1.6 }}>
                                  ...
                                  <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                                    ...
                                    <button
                                      onClick={() => {
                                        setCanceledIds((prev) => new Set([...prev, r.id]));
                                        setCancelingId(null);
                                      }}
                                      style={{
                                        display: "inline-flex",
                          ...
                          {isCanceled && (
                            <div style={{ marginTop: "0.75rem", paddingTop: "0.6rem", borderTop: "1px solid rgba(180,140,80,0.12)" }}>
                              <span style={{ fontSize: "0.75rem", color: "#a03020", fontWeight: 500 }}>
                                この予約はキャンセル済みです。
                              </span>
                            </div>
                          )}
