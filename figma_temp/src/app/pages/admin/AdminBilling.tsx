...
import {
  FaCreditCard, FaSearch, FaEye, FaTimes,
  FaUser, FaBed, FaYenSign, FaDog, FaConciergeBell, FaStar,
...
interface ExperienceItem { name: string; price: number; priceNote: string; }

interface PriceRule {
  id: string; name: string; discountPercent: number;
  hasPeriod: boolean; periodStart: string; periodEnd: string;
  hasGuestRange: boolean; guestMin: number | null; guestMax: number | null;
  noExperienceOptions: boolean; noSupportPlan: boolean;
...
const mockBillings: BillingRecord[] = [
  {
    id: "BIL-001", reservationId: "RSV-001",
    memberName: "山田 太郎", memberEmail: "yamada@example.com", memberPhone: "090-1234-5678",
    checkIn: "2026-03-15", checkOut: "2026-03-17", nights: 2, guests: 3,
    hasPet: "small1", petBreed: "トイプードル", supportFee: true,
    experiences: [{ name: "星空ガイド", price: 2000, priceNote: "¥2,000/組" }],
    ...
    amount: 65000, status: "paid", paidAt: "2026-03-10", dueDate: "2026-03-14", createdAt: "2026-02-20",
  },
  {
    id: "BIL-002", reservationId: "RSV-002",
    memberName: "佐藤 花子", memberEmail: "sato@example.com", memberPhone: "090-2345-6789",
    checkIn: "2026-03-20", checkOut: "2026-03-22", nights: 2, guests: 2,
    hasPet: "none", supportFee: true, experiences: [],
    ...
    amount: 64800, status: "unpaid", dueDate: "2026-03-19", createdAt: "2026-02-25",
  },
  {
    id: "BIL-003", reservationId: "RSV-003",
    memberName: "鈴木 一郎", memberEmail: "suzuki@example.com", memberPhone: "090-3456-7890",
    checkIn: "2026-03-25", checkOut: "2026-03-27", nights: 2, guests: 4,
    hasPet: "large1", petBreed: "ゴールデンレトリバー", supportFee: false,
    experiences: [{ name: "BBQグリルレンタル", price: 3500, priceNote: "¥3,500/回" }],
    breakdown: { baseAmount: 40000, guestExtra: 0, petFee: 7000, supportFee: 0, transferSurcharge: 0, experiencesTotal: 3500, deposit: 10000 },
    amount: 60500, status: "unpaid", dueDate: "2026-03-24", createdAt: "2026-03-01",
  },
  {
    id: "BIL-004", reservationId: "RSV-004",
    memberName: "田中 美咲", memberEmail: "tanaka@example.com", memberPhone: "090-4567-8901",
    ...
    amount: 42000, status: "paid", paidAt: "2026-03-20", dueDate: "2026-03-31", createdAt: "2026-03-02",
  },
  {
    id: "BIL-005", reservationId: "RSV-005",
    memberName: "高橋 健一", memberEmail: "takahashi@example.com", memberPhone: "090-5678-9012",
    checkIn: "2026-04-05", checkOut: "2026-04-08", nights: 3, guests: 5,
    hasPet: "small2", petBreed: "チワワ（2頭）", supportFee: true, experiences: [],
    ...
    amount: 95000, status: "refunded", paidAt: "2026-03-03", dueDate: "2026-04-04",
    note: "天候不良のためキャンセル・全額返金", createdAt: "2026-03-03",
  },
  {
    id: "BIL-006", reservationId: "RSV-006",
    memberName: "小林 太郎", memberEmail: "", memberPhone: "090-9999-8888",
    checkIn: "2026-04-10", checkOut: "2026-04-12", nights: 2, guests: 2,
    hasPet: "none", supportFee: true,
    experiences: [{ name: "BBQグリルレンタル", price: 3500, priceNote: "¥3,500/回" }],
    breakdown: { baseAmount: 52000, guestExtra: 0, petFee: 0, supportFee: 8000, transferSurcharge: 0, experiencesTotal: 3500, deposit: 10000 },
    amount: 73500, status: "unpaid", dueDate: "2026-04-09", createdAt: "2026-03-04",
  },
];
...
export function AdminBilling() {
  ...
  const [editStatus, setEditStatus]   = useState<BillingRecord["status"]>("unpaid");
  ...
  const filtered = useMemo(() => billings.filter(b => {
    const matchSearch = !searchQuery || b.memberName.includes(searchQuery) || b.id.includes(searchQuery) || b.reservationId.includes(searchQuery);
  ...
  const totalUnpaid   = billings.filter(b => b.status === "unpaid").reduce((s, b) => s + b.amount, 0);
  const totalRefunded = billings.filter(b => b.status === "refunded").reduce((s, b) => s + b.amount, 0);

  const openDetail = (b: BillingRecord) => {
    setSelected(b);
  ...
  const detailApplicableRules = useMemo(() => {
    ...
    return priceAdjustmentRules.filter(rule => {
      ...
      if (rule.hasGuestRange) {
        if (rule.guestMin !== null && selected.guests < rule.guestMin) return false;
        if (rule.guestMax !== null && selected.guests > rule.guestMax) return false;
      }
      if (rule.hasPeriod && selected.checkIn) {
        if (selected.checkIn < rule.periodStart || selected.checkIn > rule.periodEnd) return false;
      }
      return true;
    });
  }, [selected]);
  ...
  const handleAdjustmentSave = () => {
    if (!selected) return;
    const prevAdj = selected.breakdown.adjustment ?? 0;
    const newAmount = selected.amount - prevAdj + editAdjustment;
    ...
    setBillings(prev => prev.map(b => b.id === selected.id ? updated : b));
    setSelected(updated);
  ...
  const handleStatusSave = () => {
    ...
    setBillings(prev => prev.map(b =>
      b.id === selected.id
        ? { ...b, status: editStatus, paidAt: editStatus === "paid" ? (b.paidAt ?? new Date().toISOString().split("T")[0]) : b.paidAt }
        : b
    ));
        ...
        <div className="bg-white rounded-xl border border-gray-200">
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base text-gray-900">支払一覧</h2>
                <p className="text-xs text-gray-500">全{billings.length}件 | 検索結果: {filtered.length}件</p>
              </div>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-4 py-3 text-xs text-gray-500">請求ID</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs text-gray-500">期限日</th>
                  <th className="text-left px-4 py-3 text-xs text-gray-500">支払日</th>
              ...
              <tbody>
                {filtered.map(b => (
                  <tr key={b.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">{b.reservationId}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{b.memberName}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{b.checkIn}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{b.checkOut}</td>
                    <td className="px-4 py-3 text-sm text-gray-900 text-right font-medium">¥{b.amount.toLocaleString()}</td>
                    <td className="px-4 py-3">
                      {(() => {
                        const rule = b.breakdown.adjustmentRuleId
                          ? priceAdjustmentRules.find(p => p.id === b.breakdown.adjustmentRuleId)
                          : null;
                        const adj = b.breakdown.adjustment ?? 0;
                        if (rule) {
                          return (
                        ...
                        if (adj !== 0) {
                          return (
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-xs font-medium whitespace-nowrap ${adj < 0 ? "text-red-500" : "text-green-600"}`}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{b.paidAt || "−"}</td>
        ...
        >
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white rounded-t-xl z-10">
              <div>
                <p className="text-xs text-gray-400 mb-0.5">請求ID: {selected.id} ／ 予約ID: {selected.reservationId}</p>
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-base text-gray-900">{selected.memberName}</h3>
            ...
            <div className="px-6 py-5 space-y-6">
              ...
              <Section icon={<FaUser className="w-3 h-3" />} title="宿泊者情報">
                <div className="bg-gray-50 rounded-lg px-4 py-1">
                  <InfoRow label="氏名" value={selected.memberName} />
                  <InfoRow label="メール" value={selected.memberEmail || <span className="text-gray-400">−</span>} />
                  <InfoRow label="電話番号" value={selected.memberPhone} />
                  <InfoRow label="請求作成日" value={selected.createdAt} />
                </div>
              </Section>
              ...
              <Section icon={<FaBed className="w-3 h-3" />} title="宿泊内容">
                <div className="bg-gray-50 rounded-lg px-4 py-1">
                  <InfoRow label="チェックイン" value={selected.checkIn} />
                  <InfoRow label="チェックアウト" value={selected.checkOut} />
                  <InfoRow label="泊数 / 人数" value={`${selected.nights}泊 / ${selected.guests}名`} />
                  <InfoRow label="ペット" value={petLabel(selected.hasPet, selected.petBreed)} />
                  <InfoRow label="滞在サポート" value={selected.supportFee ? "あり" : "なし"} />
                  {selected.note && <InfoRow label="備考" value={selected.note} />}
                </div>
              </Section>
                ...
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  <div className="divide-y divide-gray-100">
                    <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                      ...
                      <span className="text-gray-900">¥{selected.breakdown.baseAmount.toLocaleString()}</span>
                    </div>
                    {selected.breakdown.guestExtra > 0 && (
                      <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                        <span className="text-gray-600">追加人数料金（6名以上）</span>
                        <span className="text-gray-900">¥{selected.breakdown.guestExtra.toLocaleString()}</span>
                      </div>
                    )}
                    {selected.breakdown.petFee > 0 && (
                      <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                        <div className="flex items-center gap-1.5 text-gray-600">
                          <FaDog className="w-3 h-3" /><span>ペット料金</span>
                        </div>
                        <span className="text-gray-900">¥{selected.breakdown.petFee.toLocaleString()}</span>
                      </div>
                    )}
                    {selected.breakdown.supportFee > 0 && (
                      <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                        <div className="flex items-center gap-1.5 text-gray-600">
                          <FaConciergeBell className="w-3 h-3" /><span>滞在サポート料</span>
                        </div>
                        <span className="text-gray-900">¥{selected.breakdown.supportFee.toLocaleString()}</span>
                      </div>
                    )}
                    {selected.breakdown.transferSurcharge > 0 && (
                      <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                        <span className="text-gray-600 pl-4">└ 送迎追加（5名以上）</span>
                        <span className="text-gray-900">¥{selected.breakdown.transferSurcharge.toLocaleString()}</span>
                      </div>
                    )}
                    {selected.breakdown.experiencesTotal > 0 && (
                      <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                        <div className="flex items-center gap-1.5 text-gray-600">
                          <FaStar className="w-3 h-3" /><span>体験オプション計</span>
                        </div>
                        <span className="text-gray-900">¥{selected.breakdown.experiencesTotal.toLocaleString()}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center px-4 py-2.5 text-sm bg-gray-50">
                      ...
                      <span className="text-gray-700">
                        ¥{(
                          selected.breakdown.baseAmount + selected.breakdown.guestExtra +
                          selected.breakdown.petFee + selected.breakdown.supportFee +
                          selected.breakdown.transferSurcharge + selected.breakdown.experiencesTotal
                        ).toLocaleString()}
                      </span>
                    </div>
                    {(selected.breakdown.adjustment ?? 0) !== 0 && (() => {
                      const rule = selected.breakdown.adjustmentRuleId
                        ? priceAdjustmentRules.find(p => p.id === selected.breakdown.adjustmentRuleId)
                        : null;
                      const adj = selected.breakdown.adjustment ?? 0;
                      return (
                        <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                          <div className="flex items-center gap-2 text-purple-700">
                            ...
                            <span>{rule ? `${rule.name}（-${rule.discountPercent}%）` : (selected.breakdown.adjustmentNote || "手動調整")}</span>
                          </div>
                    ...
                    <div className="flex justify-between items-center px-4 py-2.5 text-sm">
                      <span className="text-gray-600">
                        保証料<span className="ml-1.5 text-xs text-green-600 font-medium">返金制</span>
                      </span>
                      <span className="text-gray-900">¥{selected.breakdown.deposit.toLocaleString()}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-center px-4 py-3.5 bg-[#0a2105]">
                    ...
                    <span className="text-white text-xl font-bold tracking-tight">¥{selected.amount.toLocaleString()}</span>
                  </div>
                </div>
                    ...
                    {detailApplicableRules.length === 0 ? (
                      ...
                      <div className="space-y-1">
                        {detailApplicableRules.map(rule => {
                          const accommodationBase = selected.breakdown.baseAmount + selected.breakdown.guestExtra;
                          ...
                          const blockedByOptions =
                            (rule.noExperienceOptions && selected.breakdown.experiencesTotal > 0) ||
                            (rule.noSupportPlan && selected.breakdown.supportFee > 0);
                          return (
                            <button
                              key={rule.id}
                              type="button"
                              ...
                              title={blockedByOptions
                                ? [
                                    rule.noExperienceOptions && selected.breakdown.experiencesTotal > 0 ? "体験オプションが含まれているため適用不可" : "",
                                    rule.noSupportPlan && selected.breakdown.supportFee > 0 ? "滞在サポートが含まれているため適用不可" : "",
                                  ].filter(Boolean).join(" / ")
                                : undefined}
                  ...
                  {editAdjustment !== 0 && (
                    <div className="flex justify-between text-xs text-gray-500">
                      ...
                      <span className="font-semibold text-gray-800">
                        ¥{(selected.amount - (selected.breakdown.adjustment ?? 0) + editAdjustment).toLocaleString()}
                      </span>
                    </div>
                  )}
              ...
              {/* 支払情報 */}
              <Section icon={<FaCreditCard className="w-3 h-3" />} title="支払情報">
                <div className="bg-gray-50 rounded-lg px-4 py-1">
                  <InfoRow label="現在の状態" value={statusBadge(selected.status)} />
                  <InfoRow label="支払期限" value={selected.dueDate} />
                  <InfoRow label="支払日" value={selected.paidAt ?? <span className="text-gray-400">未払い</span>} />
                </div>
              </Section>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  <div className="flex gap-2">
                    {(["unpaid", "paid", "refunded"] as const).map(s => {
                      const cfg = {
                        unpaid:   { label: "未払い",   cls: "border-orange-300 bg-orange-50 text-orange-800" },
                        paid:     { label: "支払済",   cls: "border-blue-300 bg-blue-50 text-blue-800" },
                        refunded: { label: "返金済",   cls: "border-gray-300 bg-gray-100 text-gray-700" },
                      }[s];
                      const isActive = editStatus === s;
                      return (
                        <button
                          key={s}
                          onClick={() => { setEditStatus(s); setStatusSaved(false); }}
                  ...
                  <button
                    ...
                    disabled={editStatus === selected.status && !statusSaved}
                    className="w-full py-2.5 rounded-lg text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed bg-[#0a2105] text-white hover:bg-[#071a04]"
                  >
                    {statusSaved
                      ? <span className="flex items-center justify-center gap-1.5"><FaCheckCircle className="w-3.5 h-3.5" /> 保存しました</span>
