...
const EXP_MAP: Record<string, { perPerson: boolean; amount: number }> = {
  "田植え体験":      { perPerson: true,  amount: 4500 },
  "稲刈り体験":      { perPerson: true,  amount: 4500 },
  "薪割り体験":      { perPerson: false, amount: 2000 },
  "夏野菜収穫体験":  { perPerson: false, amount: 1500 },
  "BBQグリルレンタル": { perPerson: false, amount: 3500 },
  "星空ガイド":      { perPerson: false, amount: 2000 },
};
...
export function ReservationConfirm() {
  ...
  const transferSurcharge = form.supportPlan === "yes" && guestsNum >= 5 ? 5000 : 0;
  ...
  const deposit    = 10000;
  const grandTotal = baseTotal + guestExtra + petTotal + supportFee + transferSurcharge + expTotal + deposit;
  ...
  const priceRows: { label: string; amount: number; note?: string }[] = [
    ...
    ...(supportFee > 0
      ...
      : []),
    ...(transferSurcharge > 0
      ? [{ label: "送迎追加（5名以上・一律）", amount: transferSurcharge, note: "追加車両手配費" }]
      : []),
    ...(expTotal > 0
  ...
  const handleConfirm = () => {
    const bookingRef = `ELB-${Date.now().toString(36).toUpperCase().slice(-6)}`;
    navigate("/reservation/complete", {
      state: { form, checkin, checkout, nights, dayType, grandTotal, bookingRef },
    });
    window.scrollTo(0, 0);
  };
                    ...
                    {form.experiences.map((exp) => (
                      <Row key={exp} label={exp} value={
                        (() => {
                          ...
                          if (!info) return "";
                          if (info.perPerson) return `¥${(info.amount * guestsNum).toLocaleString()}（¥${info.amount.toLocaleString()} × ${guestsNum}名）`;
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      <span style={{ fontSize: "0.78rem", color: "#8a7868", flexShrink: 0 }}>{label}</span>
      <span style={{ fontSize: "0.85rem", fontWeight: 600, color: "#2c1e10", textAlign: "right" }}>
        {value}
      </span>
    </div>
  );
