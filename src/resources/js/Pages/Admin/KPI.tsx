...
function MemberStatusChart({ data }: { data: MemberStatus[] }) {
    return (
        <ResponsiveContainer width="100%" height={300}>
            <PieChart>
                <Pie
...
export default function KPI({ monthlySales, memberStatusData }: Props) {
    ...
    const avgRepeaterRate = (
        repeaterData.reduce((sum, item) => sum + item.repeaterRate, 0) /
        repeaterData.length
    ).toFixed(1);
    const avgSatisfaction = (
        satisfactionData.reduce((sum, item) => sum + item.score, 0) /
        satisfactionData.length
    ).toFixed(1);
    const sortedExperiences = [...experiencePopularity].sort(
                                ...
                                {sortedExperiences.map((experience, index) => (
                                    ...
                                    >
                                        <td className="px-4 py-3 text-center">
                                            <span
                                                className={`w-6 h-6 inline-flex items-center justify-center rounded-full text-xs ${
                                                    index === 0
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="w-32 bg-gray-200 rounded-full h-2">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${(experience.count / maxCount) * 100}%`,
                                                        }}
                                                    />
                                                </div>
