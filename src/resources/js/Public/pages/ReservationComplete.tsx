...
export function ReservationComplete() {
    const { user } = useAuth();
    const { state: navigationState } = useLocation() as {
        state: ReservationCompleteState | null;
    };
    const { flash = {} } = usePage().props as unknown as {
        flash?: { reservationComplete?: ReservationCompleteState };
    };
    const state = navigationState ?? flash.reservationComplete ?? null;
    ...
    const { form, checkin, checkout, nights, dayType, grandTotal, bookingRef } =
        state;
    ...
    const summaryItems = [
        {
            icon: <FaCalendarAlt size={13} color="#1e3c0e" />,
            label: "チェックイン",
        ...
        {
            icon: <FaUsers size={13} color="#1e3c0e" />,
            label: "宿泊人数",
            value: `${guestsNum}名`,
        },
        ...(form.pets !== "none"
            ? [
                  {
                      icon: <FaPaw size={13} color="#1e3c0e" />,
                      label: "ペット",
                      value: petLabel,
                  },
              ]
        ...
        ...(form.experiences.length > 0
            ? [
                  {
                      icon: <FaMountain size={13} color="#1e3c0e" />,
                      label: "体験オプション",
                      value: form.experiences.join("、"),
                  },
              ]
            : []),
    ];
            ...
            >
                <div style={{ maxWidth: "680px", margin: "0 auto" }}>
                    ...
                    >
                        ...
                        >
                            2〜3営業日以内に <strong>{user?.email}</strong>{" "}
                            宛にご連絡いたします。
                        </p>
                        ...
                        >
                            ...
                            >
                                ・ご予約内容の詳細は確認メールにてご案内いたします
                                <br />
                                ・保証料（¥10,000）はご滞在後にトラブルがなければ全額返金されます
                                <br />
